...
export default function OverviewTab() {
  ...
  const cards = [
    ...
    { label: "Total Debtors (owed to you)", value: outstanding?.totalDebtors, icon: ArrowDownCircle, color: "text-amber-600", tip: "Debtors are customers or parties who still owe you money — e.g. an unpaid sale or an advance you gave out." },
    { label: "Total Creditors (you owe)", value: outstanding?.totalCreditors, icon: ArrowUpCircle, color: "text-red-600", tip: "Creditors are suppliers or parties you still owe money to — e.g. an unpaid purchase bill." },
    { label: "Net Profit (this month)", value: pl?.netProfit, icon: TrendingUp, color: (pl?.netProfit ?? 0) >= 0 ? "text-emerald-600" : "text-red-600", tip: "Total income minus total expense for the current month, calculated live from all posted vouchers." },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <PageHelpDialog
        ...
        description="Your shop's full double-entry books. Every sale, purchase, girvi transaction, repair, custom order, and karigar payment posts here automatically — you rarely need to enter anything manually."
        sections={[
          {
            heading: "The other Accounting tabs",
            items: [
              "Chart of Accounts — the list of every account (Cash, Bank, Sales, individual customers/suppliers, etc.) your entries can post against",
              "Journal Vouchers — for one-off manual entries (rent, salary, misc expenses) or corrections",
              "Ledgers & Books — any account or party's running balance, plus the Day Book, Cash Book, and Bank Book",
              "Reports — Trial Balance, Profit & Loss, Balance Sheet, and Outstanding (Debtors/Creditors)",
            ],
          },
          {
            heading: "Terms you'll see",
            items: [
              "Debtors — customers/parties who owe you money",
              "Creditors — suppliers/parties you owe money to",
              "Ledger — the running transaction history of one account",
              "Journal Voucher — a single balanced accounting entry (debits = credits)",
            ],
          },
        ]}
      />
