...
export default function Billing() {
  ...
  // Bug fix: save customer separately so it's available on the success screen
  const [saleComplete, setSaleComplete] = useState<{ invoiceNumber: string; total: number; customerName: string; customerMobile: string } | null>(null);
  ...
  const completeSale = () => {
    ...
    createSale.mutate({
      ...
      onSuccess: (sale) => {
        // Bug fix: save customer info BEFORE clearing state so WhatsApp button works
        setSaleComplete({
  ...
  const sendInvoiceWhatsApp = () => {
    if (!saleComplete || !saleComplete.customerMobile) return;
    const msg = `Dear ${saleComplete.customerName}, your invoice ${saleComplete.invoiceNumber} for ${formatCurrency(saleComplete.total)} has been generated. Thank you for shopping with us!`;
      ...
      <div className="max-w-lg mx-auto mt-8 md:mt-16 text-center space-y-6 px-4">
        ...
        <div className="flex gap-3 justify-center flex-wrap">
          ...
          {saleComplete.customerMobile && (
            <Button variant="outline" className="gap-2 text-green-400 border-green-500/40 hover:bg-green-500/10" onClick={sendInvoiceWhatsApp} data-testid="button-wa-invoice">
                  ...
                  {customerSearch && (customers ?? []).length > 0 && (
                    <div className="absolute top-full left-0 right-0 z-10 mt-1 bg-card border border-border rounded-lg shadow-xl max-h-48 overflow-y-auto">
                      ...
                      {(customers ?? []).slice(0, 6).map(c => (
                        <div
                          key={c.id}
                          className="px-3 py-2 text-sm cursor-pointer hover:bg-muted/20 flex justify-between gap-2"
                  ...
                  <tbody>
                    {cart.map(item => (
                      <tr key={item.inventoryItemId} className="border-b border-border/50">
                        <td className="px-4 py-2.5 font-medium max-w-[120px] truncate">{item.itemName}</td>
                        <td className="px-4 py-2.5 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <button onClick={() => setCart(prev => prev.map(c => c.inventoryItemId === item.inventoryItemId ? { ...c, quantity: Math.max(1, c.quantity - 1) } : c))} className="w-5 h-5 rounded bg-muted text-foreground flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-colors">-</button>
                            <span className="w-5 text-center">{item.quantity}</span>
                            <button onClick={() => setCart(prev => prev.map(c => c.inventoryItemId === item.inventoryItemId ? { ...c, quantity: c.quantity + 1 } : c))} className="w-5 h-5 rounded bg-muted text-foreground flex items-center justify-center hover:bg-primary hover:text-primary-foreground transition-colors">+</button>
                          </div>
                        </td>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between items-center gap-2">
                  ...
                  <input
                    ...
                    className="w-24 text-right bg-background border border-border rounded px-2 py-1 text-sm focus:border-primary outline-none"
